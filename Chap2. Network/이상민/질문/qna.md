...
- **HTTP/3에서는 어떻게 보안 문제를 처리하는가?**
    
    QUIC기반으로 동작하는데 이때 TLS 계층없이는 동작하지 않는다. (보안 연결을 강제한다.)
    

---

- **DHCP는 무엇이고 어떤 과정을 거쳐서 IP를 얻는가?**
    
    동적으로 IP주소를 할당해주는 것을 의미한다. 동적으로 할당하는 이유는 기기를 사용하지 않을때도 IP주소를 할당한다면 이는 IP주소의 낭비로 이어지기 때문에 기기를 사용할때만 동적으로 IP를 할당하는 것으로 IP주소의 낭비를 막는다.
    
    1. 호스트가 DHCP 서버를 찾기 위해 브로드 캐스팅함
    2. DHCP 서버는 브로드캐스팅으로 자신의 정보를 알림
    3. 호스트는 받은 DHCP 서버중 하나를 선택해서 IP를 받는다

---

- **NAT가 뭔지**
    
    사설 네트워크에서 사용하는 IP를 공인 IP로 바꿔주는 과정을 의미한다. 
    
    1. 사설 네트워크에서 외부 네트워크로 요청 전송시 라우터에서 출발지 IP 주소 변환
    2. 라우터는 다시 되돌리기 위해 변환한 주소의 쌍을 NAT 테이블에 저장
    3. 요청에 대한 응답이 라우터로 돌아오면 목적지 IP주소를 NAT 테이블로 참조하여 변환

---

- **디피 헬만 키 교환 암호화 알고리즘**
    
    디피-헬만 키 교환 암호화 알고리즘은 암호화 키를 교환하는 하나의 방법이다.
    
    $$
    y=g^xmodp
    $$
    
    g(원시 값, 사용자 모두 동일한 g값을 사용한다.)와 x(사용자들의 개인 키)와 p(소수)를 안다면 y(공개 키)는 구하기 쉽지만 g(원시 값, 사용자 모두 동일한 g값을 사용한다.)와 y(공개 키)와 p(소수)만 안다면 x(사용자들의 개인 키)를 구하기는 어렵다는 원리에 기반한 알고리즘입니다.
    

---

- **HTTPS를 구현하는게 SEO에 도움되는 이유**
    
    검색엔진들이 검색 결과 최적화를 할때 HTTPS인 적용한 사이트를 적용하지 않은 사이트보다 더 높은 우선순위에 두기 때문이다.
...
- **DNS란 무엇인가요?**
    ...
    Domain Name System Servers의 약자로 도메인 이름 시스템 서버는 URL들의 이름과 IP주소를 저정하고 있는 데이터 베이스를 의미하며 IP주소 대신 사용자가 사용하기 편리하도록 주소를 매핑해주는 역할을 한다.
    

---

- **도메인 주소를 입력했을때 어떤일이 동작하는지**
    1. 사용자가 웹브라우저 검색차에 도메인 주소를 입력
    2. 웹 브라우저는 캐싱된 DNS 기록들을 통해 해당 도메인주소와 대응하는 IP주소를 확인
        - 만약 캐싱된 기록이 없을 경우 다음 단계로 넘어간다.
    3. 웹 브라우저가 HTTP를 사용하여 DNS에게 입력된 도메인 주소를 요청
    4. ISP(Internet Service Provider 인터넷 접속 서비스를 제공하는 업체들 KT, SKT, LG U+..)의 DNS 서버가 호스팅하고 있는 서버의 IP주소를 찾기 위해 DNS query를 날린다.
        ...
        - name server는 도메인 이름 구조에 기반해서 주소를 검색하게 된다.

---

- **CIDR의 정의에 대해서 이야기 해주세요**
    
    사설 IP 주소 범위를 나눌 때 사용된다. IP주소의 영역을 두 개로 나누는데, 앞을 `네트워크 영역` 그리고 뒤를 `호스트 영역`으로 나눕니다. `172.31.0.0/16`와 같이 표현되는데 여기에 나타난 /16은 앞의 몇 비트를 고정할지에 대한 정보이다.(앞의 몇 비트를 네트워크 영역으로 정할지에 대해 정한다.)